...
Supports two retrieval modes:
  ...
  - "embedding": encode the task description with the configured embedding model and rank
...
class SkillsOnlyMemory(BaseMemory):
    ...
    * ``"embedding"`` – the task description is encoded with a
      SentenceTransformer model (the configured embedding model by default).  Both
    ...
    def __init__(
        ...
        Args:
            ...
            embedding_model_path: Local path (or HF model ID) for the
                                  ...
                                  Defaults to ``"Qwen/the configured embedding model"``.
        ...
        # Backward compatibility: earlier Search-QA assets used `query_type_skills`.
        if 'task_specific_skills' not in self.skills and 'query_type_skills' in self.skills:
            self.skills['task_specific_skills'] = self.skills['query_type_skills']

        self.retrieval_mode = retrieval_mode
        self.embedding_model_path = embedding_model_path or "Qwen/the configured embedding model"
    ...
    def _detect_task_type(self, task_description: str) -> str:
        ...
        # ---- Search-QA categories ---------------------------------------
        elif {'direct_retrieval', 'multi_hop_reasoning', 'entity_attribute_lookup', 'compare'} & set(task_specific):
            if any(kw in goal for kw in [' which ', ' who is older', ' who was born first', 'earlier', 'later', 'larger', 'smaller', 'more than', 'less than']):
                return 'compare'
            if any(kw in goal for kw in ['occupation', 'architect', 'nationality', 'birthplace', 'born in', 'located in']) and any(kw in goal for kw in ['who', 'what', 'which person', 'which city', 'which country']):
                return 'entity_attribute_lookup'
            if any(kw in goal for kw in ['director', 'author', 'producer', 'parent', 'spouse', 'founded', 'invented']) or (' of the ' in goal and ' of ' in goal):
