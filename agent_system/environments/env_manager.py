...
class SearchEnvironmentManager(EnvironmentManagerBase):
    ...
    def __init__(self, envs, projection_f, config):
        ...
        if config.env.get('use_slim_memory', False):
            from slim_method.memory import SLIMMemory
            slim_cfg = config.env.slim_memory
            self.retrieval_memory = SLIMMemory(
                skills_json_path=slim_cfg.skills_json_path,
                top_k=slim_cfg.get('top_k', 3),
                lambda_utility=slim_cfg.get('lambda_utility', 0.2),
                tau_route=slim_cfg.get('tau_route', 0.35),
                tau_embedding=slim_cfg.get('tau_embedding', 0.45),
                retrieval_mode=slim_cfg.get('retrieval_mode', 'embedding'),
                embedding_url=slim_cfg.get('embedding_url', 'http://127.0.0.1:6000/v1/embeddings'),
                embedding_model=slim_cfg.get('embedding_model', 'models/embedding_model'),
                state_path=slim_cfg.get('state_path', None),
                enable_embedding_dedup=slim_cfg.get('enable_embedding_dedup', False),
                dedup_similarity_threshold=slim_cfg.get('dedup_similarity_threshold', 0.92),
            )
            self.retrieved_memories = None
            print('[SearchEnvironmentManager] SLIM memory enabled')
        elif config.env.get('use_skills_only_memory', False):
            ...
            self.retrieved_memories = None
            print(
                f"[SearchEnvironmentManager] Skills-only memory enabled "
            ...
            self.retrieval_memory = RetrievalMemory(
                ...
                embedding_model_name=config.env.retrieval_memory.get('embedding_model', 'sentence-transformers/all-MiniLM-L6-v2'),
    ...
    def reset(self, kwargs) -> Tuple[Dict[str, Any], List[Dict]]:
        ...
        if self.retrieval_memory is not None:
            ...
            if self.config.env.get('use_slim_memory', False):
                mem_config = self.config.env.slim_memory
            elif self.config.env.get('use_skills_only_memory', False):
...
class AlfWorldEnvironmentManager(EnvironmentManagerBase):
    def __init__(self, envs, projection_f, config):
        ...
        if config.env.get('use_slim_memory', False):
            ...
            self.retrieved_memories = None
            print('[AlfWorldEnvironmentManager] SLIM memory enabled')
        elif config.env.get('use_skills_only_memory', False):
            from agent_system.memory import SkillsOnlyMemory
            som_cfg = config.env.skills_only_memory
            self.retrieval_memory = SkillsOnlyMemory(
...
def make_envs(config):
    ...
    if "search" in config.env.env_name.lower():
        ...
        return envs, val_envs, test_envs
        ...
        test_files = config.data.get("test_files", None)
        if test_files is not None:
            _test_envs = build_alfworld_envs(
                alf_config_path,
            ...
            test_envs = AlfWorldEnvironmentManager(_test_envs, projection_f, config)
        return envs, val_envs, test_envs
