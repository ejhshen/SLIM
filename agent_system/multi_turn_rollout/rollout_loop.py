    ...
    def vanilla_multi_turn_loop(
        ...
        for _step in range(self.config.env.max_steps):
            ...
            if getattr(envs, 'retrieved_memories', None) is not None:
                batch.non_tensor_batch['selected_skill_ids'] = np.array(
                    [list(memory.get('selected_skill_ids', [])) for memory in envs.retrieved_memories],
                    dtype=object,
                )
                batch.non_tensor_batch['slim_task_type'] = np.array(
                    [memory.get('task_type', 'unknown') for memory in envs.retrieved_memories],
                    dtype=object,
                )
