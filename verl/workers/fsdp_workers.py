...
class ActorRolloutRefWorker(Worker):
    ...
    def __init__(self, config: DictConfig, role: str):
        ...
        print(
            f"[SLIMWorkerTiming] role={self.role} ActorRolloutRefWorker.__init__ "
            f"finished in {time.perf_counter() - ctor_start:.2f}s"
        )
    ...
    def init_model(self):
        ...
        print(f"[SLIMWorkerTiming] role={self.role} init_model start")
        ...
        if self._is_actor or self._is_rollout:
            ...
            print(
                f"[SLIMWorkerTiming] role={self.role} actor model+optimizer init "
                f"finished in {time.perf_counter() - init_start:.2f}s"
            )
        ...
        if self._is_rollout:
            ...
            print(
                f"[SLIMWorkerTiming] role={self.role} rollout init "
                f"finished in {time.perf_counter() - rollout_start:.2f}s"
            )
        ...
        if self._is_ref:
            ...
            print(
                f"[SLIMWorkerTiming] role={self.role} ref init "
                f"finished in {time.perf_counter() - ref_start:.2f}s"
            )
        ...
        print(f"[SLIMWorkerTiming] role={self.role} init_model done in {time.perf_counter() - init_start:.2f}s")
