...
class RayPPOTrainer:
    ...
    def __init__(
        ...
        self._create_dataloader(train_dataset, val_dataset, collate_fn, train_sampler, test_dataset=test_dataset)
    ...
    def _validate_once(
        ...
        target_envs = self.val_envs if envs is None else envs
        target_dataloader = self.val_dataloader if dataloader is None else dataloader
        mp = metrics_prefix

        for test_data in target_dataloader:
        ...
        if allow_skill_update and mp == 'val' and self.config.env.get('skills_only_memory', {}).get('enable_dynamic_update', False):
    ...
    def fit(self):
        ...
        if self.config.trainer.get("test_only", False):
            ...
            assert self.test_envs is not None, "test_only requires test_envs"
            test_metrics = self._validate_once(
                envs=self.test_envs,
                dataloader=self.test_dataloader,
                metrics_prefix="test",
                skip_generation_logging=False,
                allow_skill_update=False,
            )
